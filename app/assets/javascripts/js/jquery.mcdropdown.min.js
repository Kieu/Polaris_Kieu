(function($) {
	...
	$.mcDropDownMenu = function(el, list, options) {
		...
		$(document).bind("click", function(e) {
			...
			// if ($ul.length) {
				...
				// if (bIsParent && $keylist && $ul[0] === $keylist[0]) {
					...
					// return false
				// } else {
					// if (!settings.allowParentSelect && bIsParent) {
						// return false
					// }
				// }
				// if ($target.not(".mc_root")) {
					// console.log($target.parent().parent()[0]);
					// hideBranch.apply($target.parent().parent()[0], [e])
				// }
				// if (settings.click != null && settings.click.apply($target, [e, thismenu, settings]) == false) {
					// return false
				// }
			// }
			if ($target.is(".mc_root")) {
				return false;
			}
			thismenu.closeMenu()
		});
		...
		$list.appendTo("body").css({
			...
			top : -10000,
			left : -10000
		}).find("ul").andSelf().css("display", "block").each(function() {
			var $el = $(this);
		...
		$divInput = $('<div class="mcdropdown"><a href="#" tabindex="-1"></a><input type="hidden" name="1' + (el.name || el.id) + '" id="1' + (el.id || el.name) + '" /></div>').appendTo($('<div style="position: relative;"></div>')).parent();
		...
		if ($.browser.safari) {
			setTimeout(function() {
				$self.width($divInput.width() - $("a", $divInput).width())
			}, 100)
		}
		$self.width(di.width).filter(":input").attr("autocomplete", "off").bind("keypress", checkKeypress).bind("mousedown", function(e) {
		...
		$($divInput).bind("click", function(e) {
			if (bDisabled) {
				return false
			}
			thismenu.openMenu(e);
		...
		if (bInput) {
			this.setValue($self[0].defaultValue, true)
		}
		...
		function getNodeText($el) {
			...
			if (nContents[0] && nContents[0].nodeType == 3) {
				...
				if (nContents[0] && nContents[0].nodeType == 1) {
		...
		function addBindings(el) {
			removeBindings(el);
			$("> li", el).bind("mouseover", hoverOver).bind("mouseout", hoverOut)
		}

		function removeBindings(el) {
			$("> li", el).unbind("mouseover", hoverOver).unbind("mouseout", hoverOut)
		}
				...
				setTimeout(function() {
					if ($li.parent().find("> li.mc_hover").length == 0) {
						$li.parents("li").each(function() {
							...
							if ($(this).siblings().filter(".mc_hover").length > 0) {
								return false
							}
						})
					}
		...
		function hideBranch() {
			var $ul = $("> ul", this);
			if ($ul.is(":hidden") || ($ul.length == 0)) {
				return false
			}
			if (settings.dropShadow && $.data(this, "shadow")) {
				$.data(this, "shadow").hide()
			}
			if (isIE6) {
		...
		function getPadding(el, name) {
			...
			return (parseInt("0" + $.curCSS(el, "border" + torl + "Width", true), 10) + parseInt("0" + $.curCSS(el, "border" + borr + "Width", true), 10) + parseInt("0" + $.curCSS(el, "padding" + torl, true), 10) + parseInt("0" + $.curCSS(el, "padding" + borr, true), 10) + parseInt("0" + $.curCSS(el, "margin" + torl, true), 10) + parseInt("0" + $.curCSS(el, "margin" + borr, true), 10))
		}
		...
		function columnizeList($el) {
			...
			$children.each(function(i) {
				...
				$li.css({marginLeft:(widths.column*column),marginTop:marginTop,width:widths.item})[((nextItemColumn>column)||(currentItem==items))?"addClass":"removeClass"]("mc_endcol")[(marginTop!=0)?"addClass":"removeClass"]("mc_firstrow");
		...
		function onFocus(e) {
			...
			if (!$keylist) {
				...
				if (isIE6 && !!$.fn.bgIframe) {
					$keylistiframe = $("<div></div>").bgIframe().appendTo("body")
				}
			}
		...
		function onBlur(e) {
			...
			iBlurTimeout = setTimeout(function() {
				...
				if (settings.select != null) {
					settings.select.apply(thismenu, thismenu.getValue())
				}
				hideMatches();
				iBlurTimeout = null
			}, 200)
		...
		function showMatches($li, hideResults) {
			...
			if (!bCached || $keylist.is(":hidden")) {
				...
				if (isIE6) {
					...
					if ((maxHeight > 0) && (maxHeight < $keylist.outerHeight())) {
						$keylist.height(maxHeight)
					}
					if (!!$.fn.bgIframe) {
						anchorTo($divInput.parent(), $keylistiframe.css({
		...
		function hideMatches() {
			if (isIE6 && !!$.fn.bgIframe && $keylistiframe) {
				$keylistiframe.hide()
			}
			if ($keylist) {
				$keylist.hide()
				...
				if (e.keyCode == 40) {
					...
					if (e.keyCode == 27) {
						...
						if (e.keyCode == 8 || e.keyCode == 37) {
							...
							if (e.keyCode == 9 || e.keyCode == 13 || e.keyCode == 39 || key == settings.delim) {
								var $first = $current.find("> ul > li:first");
								if ($first.length > 0) {
									updateValue($first)
								} else {
									if ($.browser.msie) {
										setSelection($self[0], 0, 0)
									}
									if (e.keyCode == 9) {
		...
		function moveMatch(step) {
			...
			if ($current.length == 0) {
				$current = matchesCache.filter(".mc_hover, .mc_hover_parent")
			}
			...
			if (!matchesCache || matchesCache.length == 0 || $current.length == 0) {
				return false
			}
			pos = pos + step;
			if (pos < 0) {
				pos = matchesCache.length - 1
			} else {
				if (pos >= matchesCache.length) {
					pos = 0
				}
			}
		...
		function updateValue($li, keepTypedText) {
			...
			if (!keepTypedText) {
				typedText = treePath.join(settings.delim).toLowerCase() + (treePath.length > 0 ? settings.delim : "")
			}
		...
		function scrollIntoView($el, center) {
			...
			if ((el.offsetTop + el.offsetHeight) > (scrollable.scrollTop + scrollable.clientHeight)) {
				...
				if (el.offsetTop - s.bTop - s.bBottom <= (scrollable.scrollTop + s.pTop + s.pBottom)) {
