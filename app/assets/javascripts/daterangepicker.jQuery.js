...
//		},
	...
	var datepickerOptions = {
		onSelect: function(dateText, inst) { 
				...
				if(rangeInput.length == 2){
					...
					//display 2 range inspite of identical value
	...
	var rpPresets = false; /*hieupv2 (function(){
	...
	function fDate(date){
	   ...
	   //date format depends on Lang parameter - hieupv2
	...
	var rpPickers = jQuery('<div class="ranges ui-widget-header-1 ui-corner-all ui-helper-clearfix"><div class="range-start"><span class="title-start">Start Date</span></div><div class="range-end"><span class="title-end">End Date</span></div></div>').appendTo(rp);
	...
	jQuery(this).click(function(){
		...
		startDate[1] = "開始日";
		startDate[2] = "From date";
		
		var endDate = new Array();
		endDate[1] = "終了日";
		endDate[2] = "To date";
		
		rp.find('.title-start').text(startDate[options.lang]);
		rp.find('.title-end').text(endDate[options.lang]);
		rp.find('.range-start').restoreDateFromData().css('opacity',1).show(400);
		rp.find('.range-end').restoreDateFromData().css('opacity',1).show(400);
		setTimeout(function(){doneBtn.fadeIn();}, 400);
	...
	if(options.arrows && rangeInput.size()==1){
        ...
        function revertDate(lang) {
            var sDate = $.cookie("s");
            var eDate = $.cookie("e");
            if (lang == 2){//ENG
              sDate = _convertDate(sDate);
              eDate = _convertDate(eDate);
            }
            
            rangeInput.val(sDate+' '+ options.rangeSplitter +' '+eDate);
            rp.find('.range-start').datepicker('setDate', sDate).find('.ui-datepicker-current-day').trigger('click');
            rp.find('.range-end').datepicker('setDate', eDate).find('.ui-datepicker-current-day').trigger('click');
        }
